...
# TASK 2 specific functions
  ...
  def t2_scan_imu_module_location(self):
    ...
    self.go_to_pose_goal(0.03, 0.3, 0.09, scan_pose[3], scan_pose[4]+25, scan_pose[5])
    rospy.sleep(2)
    self.t2_extract_aruco_poses()
    self.go_to_pose_goal(0.04, 0.28, 0.12, scan_pose[3], scan_pose[4]+30, scan_pose[5])
    rospy.sleep(2)
    self.t2_extract_aruco_poses()
    self.move_cartesian(0.01, -0.03, -0.02)
    rospy.sleep(2)
    self.t2_extract_aruco_poses()
    self.move_cartesian(-0.025, 0.06, -0.02)
    rospy.sleep(2)
    self.t2_extract_aruco_poses()
